...
' );
	...
	if( $ctxProcess[ 'mode' ] == 'full' && $ctx -> aAniAppear )
	{
		...
		HtmlNd::SetValFromContent( $itemScript, str_replace( 'COMPILE_FAKE_SELECTORS_OBJECT', $itemScriptContSelectors, "(function(h,n){function p(a,k=!1){const {top:b,left:g,bottom:c,right:d}=a.getBoundingClientRect(),{innerHeight:e,innerWidth:f}=n;return k?0<=b&&0<=g&&c<=e&&d<=f:!(b>e||0>c)&&!(g>f||0>d)}function m(a){function k(g,c){function d(){for(;e<c.length;){var f=c[e++],l;p(f)&&(l=g(f));if(l){setTimeout(d,l);break}}}var e=0;d()}a=COMPILE_FAKE_SELECTORS_OBJECT;for(var b in a)k(a[b],h.querySelectorAll(b))}h.addEventListener(\"seraph_accel_calcSizes\",m,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){h.removeEventListener(\"seraph_accel_calcSizes\",\nm,{capture:!0,passive:!0})})})(document,window)" ) );
		$ctxProcess[ 'ndBody' ] -> appendChild( $itemScript );
	}
